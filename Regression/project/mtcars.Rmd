...
# Regression Modeling
I answer these questions using regression modeling. Basic approach is to describe the fuel economy as an outcome of other characteristics of the car. I am interested in models that include the transmission type, as these indicate the size of the effect on fuel economy.

## Model Selection
The model selection process began by fitting a simple model with transmission predicting economy. From here, additional candidate variables were added one at a time, with the interaction term. The variables were acceleration speed, weight, engine displacement, number of gears, horsepower, and number of carburetors. I observed one of three outcomes when including the variable:
...
The first case is useful information and I generally excluded the variable from that point forward. The second case is interesting to the question at hand. By exploring the third case, the most explanatory model of fuel economy can be found. This is weight and horsepower with interaction. This makes sense and it also makes sense that transmission choice is not particularly influential when predicting on those two terms.
...
The strict interpretation of the model is that automatic transmission is worse for fuel economy. Compared to manual, automatic means 5.28 less miles per gallon on average for cars with the same horsepower. * Give a CI*. The model also indicates that each additional horsepower rated translates into 0.059 less mpg, or 5.9 less miles per gallon per 100 hp.  *GIVE CI* The regression model, with confidence interval, is plotted in the appendix. 

###What is the uncertainty in our result?
Make appropriate inference from the model.

#Conclusions
The analysis shows that cars with automatic transmission have about $5.3 \plusminus ciBound$ mpg less fuel efficiency, controlling for horsepower. The analysis also showed that other design factors such as horsepower and weight are very influential. It appears that automatic transmissions are more often equipped on heavy and powerful cars, that is they are part of a high end set of features. Cars with manual transmission tended to be more likely on cars with smaller engines with a greater overall design focus on economy.
